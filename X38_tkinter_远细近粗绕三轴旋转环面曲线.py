...
#使用tkinter绘图: 算法设计：项道德（daode1212,daode3056），2021-09-26
root = Tk();  canvas = Canvas(width=1200, height=800, bg="#000")
canvas.pack();  x0=600;y0=400;xa=x0;yb=y0;s=0.0
#绘制背景圆：
canvas.create_oval(x0-225,y0-225,x0+225,y0+225,tag="p_" + str(0), width=1, fill="#006")
#================================================
while s<3600: #总时间控制
    s=s+0.01 #旋转速度
    for t in range(0, 3600, 5):  # 总循环多少次
        g=s+t/600;R=75
        #生成球面曲线点：
        xc=R*cos(g*28)*(cos(g*5)+2)
        yc=R*sin(g*28)*(cos(g*5)+2)
        zc=R*sin(g*5)
        #绕Z轴旋转空间点：
        ...
        xa=x3;yb=y3 #替换线段起点
    canvas.update()#画布更新
    for t in range(0, 3600, 5):# 重绘前删除已经绘制图像
