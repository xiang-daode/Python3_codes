...
for t in range(0, 3600, 1):  # 总循环多少次
    for k in range(t, t + 7):  # 每次生成多少个
        xt = 600 * (1 + 0.75 * math.cos(k / 36) * math.cos(k / 9))
        yt = 400 * (1 + 0.75 * math.sin(k / 36) * math.cos(k / 9))
        k2 = k - 1
        xt2 = 600 * (1 + 0.75 * math.cos(k2 / 36) * math.cos(k2 / 9))
        yt2 = 400 * (1 + 0.75 * math.sin(k2 / 36) * math.cos(k2 / 9))
        #rgb = "#" + str(hex(rnd.randint(0x100000, 0xFF00FF)))[2:]  # 生成随机颜色
        rgb = "#" + str(hex(0x100000+t*1234))[2:]  # 生成随机颜色
        canvas.create_line(
