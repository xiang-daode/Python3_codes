...
#定义画布:
root = tk.Tk();root.title("[python+tkinter]动画设计器")
w = tk.Canvas(root, width=1200, height=600, bg="#000");w.pack()
#定义中心坐标:
x0, y0 = 600, 300
#绘制标签:
lb = tk.Label(text="代码编写:  daode1212, 2021-04-13", fg="#003399", bg="#FFEE88")
...
def myDrawE():
    ...
    for g in range(-200,200,1):
        ...
        rgb ='#'+ str(hex(rnd.randint(0x888888, 0xFFFFFF)))[2:]  # 生成随机颜色
        while u > 0:
            x1, y1 = r * math.cos((g+u+3) / 23), r * math.sin((g-u+3) / 23.01)
            x2, y2 = r * math.cos((g+u) / 23), r * math.sin((g-u) / 23.01)
            x3=x1*math.cos(g / 45)-y1*math.sin(g / 45)
            y3=x1*math.sin(g / 45)+y1*math.cos(g / 45)
            x4=x2*math.cos(g / 45)-y2*math.sin(g / 45)
            y4=x2*math.sin(g / 45)+y2*math.cos(g / 45)
...
#布局各按钮:
tk.Button(root, text="[按钮A:左右移动]", command=(lambda: myDrawA()),
compound="center").pack(side="left")

tk.Button(root, text="[按钮B:上下移动]", command=(lambda: myDrawB()),
compound="center").pack(side="left")

tk.Button(root, text="[按钮C:二维旋转]", command=(lambda: myDrawC()),
compound="center").pack(side="left")

tk.Button(root, text="[按钮D:变形与旋转]", command=(lambda: myDrawD()),
compound="center").pack(side="left")

tk.Button(root, text="[按钮E:?????]", command=(lambda: myDrawE()),
compound="center").pack(side="left")
